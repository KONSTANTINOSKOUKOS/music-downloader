<template>
    <div class="cont">
        ...
        <div :style="{ backgroundColor: state.color.rgb }" class="item" color="warning">
            ...
            <ion-range :max="state.track.duration" class="range"></ion-range>
        </div>
    </div>
</template>
...
onMounted(() => {
    ...
    setInterval(() => {
        ...
        console.log(range.value.value);
        console.log(state.track.duration);
...
const change = (e: IonRangeCustomEvent<RangeChangeEventDetail>) => {
    if (!audio.value) return;
    audio.value.currentTime = (e.detail.value as number * 100);
}
</script>
<style scoped>
.cont {
